...
namespace RecapConverter
{
    public class ConverterForm : Form
    {
        ...
        {
            this.Text = "Recap Converter (Archive Tool)";
            this.Size = new Size(600, 560);    
            this.FormBorderStyle = FormBorderStyle.FixedSingle;
            this.MaximizeBox = false;

            var lblPath = new Label { Text = "Папка с данными (.sch):", Location = new Point(10, 15), AutoSize = true };
            txtInputFolder = new TextBox { Location = new Point(10, 35), Width = 480, ReadOnly = true };
            btnBrowse = new Button { Text = "Обзор", Location = new Point(500, 34), Width = 75 };
            btnBrowse.Click += (s, e) => SelectFolder();

            var grpSettings = new GroupBox { Text = "Настройки конвертации", Location = new Point(10, 70), Size = new Size(565, 120) };

            var lblRes = new Label { Text = "Разрешение:", Location = new Point(15, 30), AutoSize = true };
            ...
            var lblFps = new Label { Text = "FPS:", Location = new Point(270, 30), AutoSize = true };
            numFps = new NumericUpDown { Location = new Point(310, 28), Minimum = 1, Maximum = 60, Value = 1, Width = 50 };

            var lblEncoder = new Label { Text = "Кодек:", Location = new Point(15, 70), AutoSize = true };
            ...
            var lblQuality = new Label { Text = "Качество (CQ/CRF):", Location = new Point(320, 70), AutoSize = true };
            numQuality = new NumericUpDown { Location = new Point(430, 68), Minimum = 0, Maximum = 51, Value = 35, Width = 50 };
            var lblQualityHint = new Label { Text = "(меньше = лучше)", Location = new Point(485, 70), AutoSize = true, ForeColor = Color.Gray };
            ...
            btnConvert = new Button { Text = "НАЧАТЬ КОНВЕРТАЦИЮ", Location = new Point(10, 200), Size = new Size(565, 40), BackColor = Color.LightGreen };
            btnConvert.Click += (s, e) => StartConversion();

            progressBar = new ProgressBar { Location = new Point(10, 250), Size = new Size(565, 20) };
            lblStatus = new Label { Text = "Ожидание...", Location = new Point(10, 275), AutoSize = true };
        ...
        private async void StartConversion()
        {
            ...
            {
                MessageBox.Show("Выберите корректную папку.", "Ошибка", MessageBoxButtons.OK, MessageBoxIcon.Error);
                return;
            }

            if (!File.Exists("ffmpeg.exe"))
            {
                MessageBox.Show("ffmpeg.exe не найден в папке с программой!", "Ошибка", MessageBoxButtons.OK, MessageBoxIcon.Error);
                return;
            }
            ...
            try
            {
                ...
                if (files.Length == 0)
                {
                    OnLog("Файлы .sch не найдены.");
                    return;
                }

                OnLog($"Найдено файлов: {files.Length}. Кодировщик: {(useNvenc ? "NVIDIA" : "CPU")}, FPS: {fps}");
                ...
                OnLog("=== ГОТОВО ===");
                MessageBox.Show("Все задачи выполнены.", "Успех", MessageBoxButtons.OK, MessageBoxIcon.Information);
            }
            catch (Exception ex)
            {
                OnLog($"КРИТИЧЕСКАЯ ОШИБКА: {ex.Message}");
